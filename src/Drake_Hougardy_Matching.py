    ...
    for (u,v,d) in edges:
        for node in [u,v]:
            if matching.has_node(node) :
                for k, kd in matching[node].items():
                    key=str(sorted([k,node]))
                    if key in computedEdges:
                        computedEdges[key]=1
...
#Returns all the edges adjacent to 'edges' in matching. Excludes the weight of the
# 'center' edge irrespective of if it is in matching.
...
def maxAllowable(matching, edges, center, betaValue):
    ...
    for (u,v,d) in edges:
        ...
        #Check if we need to ensure this edge is not center
        if u==x or v==x :
    ...
    xResultEdgeList = [x for x in [xEdge1, xEdge2] if x is not None]
    yResultEdgeList = [x for x in [yEdge1, yEdge2] if x is not None]

    for edge in xResultEdgeList:
...
def getGoodBetaAugmentation(graph, maxNewMatching, center , betaValue):
    ...
    for (u,v,d) in graph.edges([node1, node2], data=True):
        ...
        if(not maxNewMatching.has_edge(u,v)):
            ...
            #du['inc_match_edges'] = edgeIncMatchingWC
    ...
    if (maxNewMatching.has_node(node1)and maxNewMatching.has_node(node2) and not maxNewMatching.has_edge(node1,node2)) :
        ...
        key1 = list(centerMatchEdge1.keys())[0]
        key2 = list(centerMatchEdge2.keys())[0]
    ...
    for v, d in aPossibleEdges.items():
...
def multi_improve_matching(graph, exactMaxMatchingWeight=None):
    ...
    while currMatchingWeight/exactMaxMatchingWeight < 0.67:     #   (2/3 - epsilon) ratio
        iteration += 1
        #print("Before Improve Matching weight: " + str(currMatchingWeight))
        ...
        newBetaWeight, betaValue=getNextBetaValue(newBetaWeight)
...
if __name__ == "__main__":
        ...
        print("\nImproved weight : " + str(improvedWeight))
